...
for (i in 1:numTestSd) {
	...
	tmp.folder		<- paste(wd, "/LmNweek/", substr(tmp.sd,4,5), "/", sep="")
	...
	if ( sum(tmp.idx) == 0 ) {
		...
		## de-trend the data
		##--------------------------------------------------------------
		tmp.hist$ws.nt	<- (tmp.hist$ws.min00 - tmp.lm$fitted.values)
		...
		##--------------------------------------------------------------
		proj.data$sf				<- seasonalFactor.vec[proj.data$nweek]
		proj.data$trend				<- predict(tmp.lm, proj.data)
		proj.data$weekly_sales.f	<- proj.data$trend + proj.data$sf
...
## Save image
##------------------------------------------------------------------
save(nweek.list, file="050.001_LmForecastNweek_20140326.Rdata")
