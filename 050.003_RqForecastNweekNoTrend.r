...
for (i in 1:numTestSd) {
	...
	tmp.folder		<- paste(wd, "/RqNweekNoTrend/", substr(tmp.sd,4,5), "/", sep="")
	...
	if ( sum(tmp.idx) == 0 ) {
		...
		##--------------------------------------------------------------
		tmp.rq		<- rq(ws.min00 ~ 1, tau=.5, data=tmp.hist, method="br")
...
## Save image
##------------------------------------------------------------------
save(nweek.list, file="050.003_RqForecastNweekNoTrend_20140326.Rdata")
