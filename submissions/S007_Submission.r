...
calcForecast <- function(myData, myDates, myResults, myMean, myMedian, myLm, myLmMulti, myStl, myTslm, myQrMulti, myFourier) {
	...
	for (i in 1:nrow(myData)) {
        ...
        ## fourier > stl > qrmulti > lmmulti > lm > tslm > median > mean ???
        if (!is.null(myFourier[[tmpSdName]])) {
...
save(S006_HistoryAndProjection, file="S007_HistoryAndProjection.Rdata")
...
write.csv(res, file="S007_Submission.csv", row.names=FALSE)
...
write.csv(misses.df, file="S007_Misses.csv")


##------------------------------------------------------------------
## Save the workspace
##------------------------------------------------------------------
save.image(file="S007_Submission.Rdata")
