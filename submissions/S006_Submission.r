...
##------------------------------------------------------------------
calcForecast <- function(myData, myDates, myResults, myMean, myMedian, myLm, myLmMulti, myStl, myTslm, myQrMulti, myFourier) {
    ...
    ##--------------------------------------------------------------
	## based on the results matrix, identify the optimal fit to use
	##--------------------------------------------------------------
    tmp.res     <- do.call(rbind, apply(myResults, 1, function(x){ which(x == min(x)) }))
	...
	for (i in 1:nrow(myData)) {
        ...
        ## find the proper row for a sd_idx in the results
        row_fl      <- ( rownames(tmp.res) == tmpSdIdx )
        
        if (sum(row_fl) == 1) {
            
            ## hit
            tmp.row     <- which( rownames(tmp.res) == tmpSdIdx )
            tmp.mod     <- tmp.res[tmp.row, 1]
            tmp.modName <- colnames(myResults)[tmp.mod]
            
            if (tmp.modName == "median") {
            ...
            } else if (tmp.modName == "fourier") {
                
                tmpProj	 <- c(myFourier[[tmpSdName]]$fitted, myFourier[[tmpSdName]]$forecast)
                dateIdx	 <- which(as.Date(myDates) == tmpDate)
                tmpFc	 <- tmpProj[dateIdx]
                tmpType	 <- "fourier"
                
            } else {
                warning("In <calcForecast> :: No model match")
            }
...
S006_HistoryAndProjection <- hp
save(S006_HistoryAndProjection, file="S006_HistoryAndProjection.Rdata")
...
write.csv(res, file="S006_Submission.csv", row.names=FALSE)
...
write.csv(misses.df, file="S006_Misses.csv")


##------------------------------------------------------------------
## Save the workspace
##------------------------------------------------------------------
save.image(file="S006_Submission.Rdata")
