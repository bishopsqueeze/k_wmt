...
## Load fit results
##------------------------------------------------------------------
load("999_CompareModels_S022_20140403.Rdata")
...
##------------------------------------------------------------------
calcForecast <- function(myData, myDates, myResults, myMean, myMedian, myLm, myLmMulti, myStl, myTslm, myQrMulti, myFourier, myVanilla, myNweek, myNweekNoTrend) {
	...
	for (i in 1:nrow(myData)) {
        ...
        ## NWEEK manual overrides
        #if ( tmpSdIdx %in% c("30_07", "33_07", "35_07", "37_07", "43_07", "44_07") ) {
        if ( tmpSdIdx %in% c() ) {
        ...
        #} else if ( tmpDept %in% c(7, 72) ) {
        } else if ( tmpDept %in% c(72) ) {
        ...
        } else if ( tmpDept %in% c() ) {
            
            tmpProj	 <- c(myNweekNoTrend[[tmpSdName]]$fitted, myNweekNoTrend[[tmpSdName]]$forecast)
            dateIdx	 <- which(as.Date(myDates) == tmpDate)
            tmpFc	 <- tmpProj[dateIdx]
            tmpType	 <- "nweek.notrend"
            
        } else if (!is.null(myVanilla[[tmpSdName]])) {
		...
		## load the forecast (forecast floored at $0)
		myForecast[i, ] <- c(tmpStore, tmpDept, max(0,tmpFc))
		myType[i] 		<- tmpType
	}
...
forecast 		<- calcForecast(    myData=forecast.test,
                                    ...
                                    myNweek=nweek_trend.list,
                                    myNweekNoTrend=nweek_notrend.list)
...
S022_HistoryAndProjection <- hp
save(S022_HistoryAndProjection, file="S022_HistoryAndProjection.Rdata")
...
write.csv(res, file="S022_Submission.csv", row.names=FALSE)
...
write.csv(misses.df, file="S022_Misses.csv")


##------------------------------------------------------------------
## Save the workspace
##------------------------------------------------------------------
save.image(file="S022_Submission.Rdata")
