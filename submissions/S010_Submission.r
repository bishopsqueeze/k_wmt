...
calcForecast <- function(myData, myDates, myResults, myMean, myMedian, myLm, myLmMulti, myStl, myTslm, myQrMulti, myFourier) {
	...
	for (i in 1:nrow(myData)) {
        ...
        ##  lmmulti > lm > tslm > median > mean > qrmulti > stl > fourier >
        if (!is.null(myLmMulti[[tmpSdName]])) {
...
save(S006_HistoryAndProjection, file="S010_HistoryAndProjection.Rdata")
...
write.csv(res, file="S010_Submission.csv", row.names=FALSE)
...
write.csv(misses.df, file="S010_Misses.csv")


##------------------------------------------------------------------
## Save the workspace
##------------------------------------------------------------------
save.image(file="S010_Submission.Rdata")
