...
calcForecast <- function(myData, myDates, myResults, myMean, myMedian, myLm, myLmMulti, myStl, myTslm, myQrMulti, myFourier) {
	...
	for (i in 1:nrow(myData)) {
        ...
        ## qrmulti > lmmulti > stl > lm > tslm > median > mean ???
        
        if (!is.null(myQrMulti[[tmpSdName]])) {
...
save(S006_HistoryAndProjection, file="S009_HistoryAndProjection.Rdata")
...
write.csv(res, file="S009_Submission.csv", row.names=FALSE)
...
write.csv(misses.df, file="S009_Misses.csv")


##------------------------------------------------------------------
## Save the workspace
##------------------------------------------------------------------
save.image(file="S009_Submission.Rdata")
